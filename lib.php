<?php
// Asegura que el script solo sea ejecutado en el contexto de Moodle. Si alguien intenta acceder directamente, se detiene la ejecución.
defined('MOODLE_INTERNAL') || die();

/**
 * Función para obtener una respuesta de ChatGPT basada en un input proporcionado por el usuario.
 *
 * @param string $input La pregunta o entrada del usuario.
 * @return string La respuesta generada por ChatGPT o un mensaje de error si ocurre algún problema.
 */
function obtener_respuesta_chatgpt($input) {
    global $CFG;

    // Obtiene la clave API almacenada en la configuración del plugin.
    $api_key = get_config('local_chatgpt_plugin', 'api_key');

    // Verifica si la clave API está configurada. Si no lo está, devuelve un mensaje de error.
    if (empty($api_key)) {
        return 'No se ha configurado la clave API.';
    }

    // URL de la API de OpenAI para las completions de chat.
    $url = 'https://api.openai.com/v1/chat/completions';

    // Base de conocimiento local para respuestas predefinidas.
    $base_conocimiento = [
        // Preguntas y respuestas frecuentes almacenadas localmente.
    ...
    if (array_key_exists($input, $base_conocimiento)) {
        // Si hay coincidencia, devuelve la respuesta correspondiente sin consultar la API.
    ...
    // Construye el contenido del mensaje que se enviará a la API, incluyendo la base de conocimiento.
    $data = array(
        'model' => 'gpt-4-turbo', // Especifica el modelo de lenguaje a utilizar.
        'messages' => array(
            array(
                'role' => 'system',
            ...
            array(
                'role' => 'user',
                'content' => $input // La entrada proporcionada por el usuario.
            )
        ),
        'max_tokens' => 150, // Máximo de tokens en la respuesta.
        'temperature' => 1, // Controla la aleatoriedad de la respuesta (1 es valor por defecto).
    );

    // Configura los encabezados de la solicitud HTTP, incluyendo la autenticación con la clave API.
    $headers = array(
        'Authorization: Bearer ' . $api_key,
        'Content-Type: application/json',
    );

    // Inicializa una sesión cURL para realizar la solicitud HTTP.
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // Indica que se debe devolver el resultado como una cadena.
    curl_setopt($ch, CURLOPT_POST, true); // Especifica que la solicitud es de tipo POST.
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($data)); // Agrega los datos de la solicitud en formato JSON.
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers); // Añade los encabezados a la solicitud.

    // Especifica el certificado CA para validar la conexión SSL (ajusta la ruta según tu servidor).
    curl_setopt($ch, CURLOPT_CAINFO, 'E:\inetpub\wwwroot\dai23\local\cacert.pem');
    ...
    if ($response === false) {
        $error_code = curl_errno($ch); // Obtiene el código de error de cURL.
        $error_msg = curl_error($ch); // Obtiene el mensaje de error de cURL.
    ...
    // Decodifica la respuesta JSON de la API en un arreglo asociativo.
    ...
    // Verifica si la respuesta contiene el contenido esperado.
    if (isset($result['choices'][0]['message']['content'])) {
        // Devuelve el contenido de la respuesta generada por ChatGPT.
        return $result['choices'][0]['message']['content'];
    } else {
        // Mensaje de error si no se pudo obtener la respuesta adecuada.
